...
import { Folder, User, FileText, Target, ArrowUp } from "lucide-react"
  ...
  return (
    <span ref={ref} className="inline-block">
      <NumberFlow
        value={displayValue}
        trend={1}
        className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground tabular-nums"
      />
    </span>
  )
}
        ...
        return (
          ...
          >
            <Card className="flex flex-col justify-between gap-3 rounded-xl border border-border/70 bg-card/70 p-4.5 sm:p-5 shadow-xs hover:border-border/90 hover:bg-card/90 transition-all duration-200 h-full">
              {/* Header: Title and Purple Icon aligned across top */}
