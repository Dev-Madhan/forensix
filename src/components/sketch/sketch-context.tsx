...
export function SketchProvider({ children }: { children: React.ReactNode }) {
  ...
  const generateSketch = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let errMsg = "Failed to generate sketch from server";
        try {
      ...
      if (data.metadata) {
        ...
        if (data.metadata.fallback_notice) {
          toast.warning("Procedural Fallback Used", {
            description: "Local GPU AI worker was unreachable. Connect your tunnel at AI_SERVICE_URL for RTX 4050 generation.",
            duration: 7000,
          });
        } else if (data.metadata.engine === "diffusion_local_sd15_controlnet") {
          toast.success("Sketch Generated by Local RTX 4050 GPU!", {
            duration: 4000,
          });
        }
      }
