...
export function CaseDetailsHeader({ caseData }: CaseDetailsHeaderProps) {
  ...
  const renderStatusBadge = (status: string) => {
    ...
    if (normalized.includes("investigation")) {
      return (
        <span className="inline-flex items-center rounded-[4px] border-2 border-[#7E22CE]/40 bg-[#2D1B4E]/80 px-2.5 py-0.5 text-xs font-semibold text-[#C084FC] tracking-wide">
          Under Investigation
        </span>
      );
    }
    if (normalized.includes("open")) {
      return (
        <span className="inline-flex items-center rounded-[4px] border-2 border-blue-500/40 bg-blue-500/15 px-2.5 py-0.5 text-xs font-semibold text-blue-400 tracking-wide">
          Open
        </span>
      );
    }
    ...
    return (
      <span className="inline-flex items-center rounded-[4px] border-2 border-border/80 bg-muted/60 px-2.5 py-0.5 text-xs font-semibold text-muted-foreground tracking-wide">
        {status}
      </span>
    );
  };
